    ...
    public function index(Request $request)
    {
        if($request->ajax()){
            ...
            ->addColumn('action', function($row){
                ...
                <a href='".route('admin.customer-edit',$row->id)."' class='edit btn btn-success btn-sm mx-1'>Edit</a>
                <a href='".route('admin.customer-delete',$row->id)."' class='delete btn btn-danger btn-sm mx-1'>Delete</a>
                </div>";
                return $actionBtn;
            })
            ->rawColumns(['action'])
